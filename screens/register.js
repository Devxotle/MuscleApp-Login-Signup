...
const sendData = async (data) => {
    try{
        await addDoc(collection(db, 'usersInfo'),(data));
    } catch (error){
        console.error(error);
    }
};
...
const Register = () => {
    ...
    const validateInput = (inputs) => {
        if (Object.values(inputs).some(value => value === null || value === '')){
            ...
            {Alert.alert('there is no null', 'check'); 
            return true;}
    };
    const onSubmit = () => {
  ...
  return (
    <SafeAreaView style={styles.mainAsset}>
        ...
        <ScrollView style={styles.scroll}>
            ...
            <TextInput style={styles.input} onChangeText={text => handleChange('Username', text)} value={inputs.Username}/>
